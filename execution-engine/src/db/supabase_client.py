"""Supabase client singleton for the Execution Engine.

Always uses service_role key — bypasses RLS for server-side operations.
Never expose this client or key to any user-facing surface.
"""
...
def _assert_service_role_configuration() -> None:
    ...
    if not key:
        raise RuntimeError(
            "SUPABASE_SERVICE_ROLE_KEY is required for execution-engine startup; "
            "refusing to fall back to anon credentials."
        )
    if key.startswith("sb_publishable_"):
        raise RuntimeError(
            "SUPABASE_SERVICE_ROLE_KEY contains a publishable/anon key; "
            "execution-engine requires the Supabase service_role secret."
        )
    ...
    try:
        ...
        if "403" in message or "Forbidden" in message:
            raise RuntimeError(
                ...
                "Check that execution-engine is loading SUPABASE_SERVICE_ROLE_KEY from its own .env."
            ) from exc
        raise RuntimeError(
            "Supabase startup self-test failed for table paper_accounts: "
            f"{message}"
